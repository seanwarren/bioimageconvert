...
void initMetaTag(TagItem *tagItem)
{
  tagItem->tagGroup = 0;  
  tagItem->tagId = 0;     
  tagItem->tagType = 0;  
...
void bim::clearMetaTag(TagItem *tagItem)
{
  if (tagItem == NULL) return;
  if (tagItem->tagLength == 0) return;
  if (tagItem->tagData == NULL) return;

  xfree( &tagItem->tagData );
  initMetaTag(tagItem);
}

void bim::clearMetaTags(TagList *tagList)
{
  if (tagList == NULL) return;
  if (tagList->count == 0) return;
  if (tagList->tags == NULL) return;

  xfree( (void **) &tagList->tags );
  tagList->count = 0;
}

bool bim::isTagPresent(TagList *tagList, int group, int tag)
{
  ...
  for (i=0; i<tagList->count; i++)
    if ( (tagList->tags[i].tagId == (uint) tag) && 
         (tagList->tags[i].tagGroup == (uint) group) ) return true;
  
  return false;
}

int bim::tagPos(TagList *tagList, int group, int tag)
{
  if (tagList == NULL) return false;
  if (tagList->count == 0) return false;
  if (tagList->tags == NULL) return false;
  ...
  for (i=0; i<tagList->count; i++)
    if  ( (tagList->tags[i].tagId == (uint) tag) &&
          (tagList->tags[i].tagGroup == (uint) group) )
    return i;
  
  return -1;
}

int bim::addMetaTag(TagList *tagList, TagItem item)
{
  uint i;
  ...
  {
    TagItem *newTags = (TagItem *) xmalloc( (tagList->count+1) * sizeof(TagItem) );
